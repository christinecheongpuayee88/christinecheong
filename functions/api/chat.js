export async function onRequestPost(context) {
  ...
  try {
    ...
    const { system, message, messages: histMessages, model, max_tokens, urls } = body;

    if (!message && !histMessages) {
    ...
    if (histMessages && Array.isArray(histMessages)) {
      // Multi-turn conversation history path
      if (system) messages.push({ role: "system", content: system });
      messages.push(...histMessages);
    } else {
      // Single-message path (existing behaviour)
